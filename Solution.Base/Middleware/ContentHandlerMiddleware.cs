...
namespace Solution.Base.Middleware
{
    ...
    public class ContentHttpHandler : BaseRangeRequestHandler
    {
        ...
        public override void ParseRequestParameters(HttpContext context)
        {
            ...
            _physicalFilePath = WebHelper.ServerDetails.GetPhysicalPath(pathName);
            if (_physicalFilePath != "")
            {
                _responseFileName = WebHelper.ServerDetails.GetFileName(_physicalFilePath);
                var folder = WebHelper.ServerDetails.GetFolderName(pathName);
                if (context.User.IsInRole("admin") || _validFolders.Where(f => pathName.StartsWith(f)).Count() > 0)
                {
                    ...
                    {
                        if (!FileHelper.DirectoryExists(WebHelper.ServerDetails.GetPhysicalPath(folder)))
                        {
                        ...
                        _physicalFilePath = WebHelper.ServerDetails.GetPhysicalPath(pathNameWithoutDashes);
        ...
        public override void CreateResponseContentIfRequired(HttpContext context)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        var converter = new Solution.Base.Infrastructure.VideoConverter(Solution.Base.Infrastructure.Server.MapContentPath(Solution.Base.Infrastructure.ConfigurationManager.AppSettings(Folders.FFMpeg)));
