...
{
    [Route("FileManager")]
    public class AdminFileManagerController : BaseControllerAuthorize
    {
        ...
        private string GetLangFile()
        {
            ...
            if (!System.IO.File.Exists(Server.MapWwwPath(filename)))
        ...
        protected Dictionary<string, string> ParseJSON(string file)
        {
            ...
            try
            {
                json = System.IO.File.ReadAllText(Server.MapWwwPath(file), System.Text.Encoding.UTF8);
            }
            catch (Exception ex) { }
        ...
        protected string GetFilesRoot()
        {
            ...
            if (ret == "")
                ret = Server.MapWwwPath("~/FileManager/Uploads");
            else
                ret = FixPath(ret);
            return ret;
        }
        ...
        protected void VerifyAction(string action)
        {
            ...
            if (Server.MapWwwPath(setting) != Server.MapWwwPath(ControllerContext.HttpContext.Request.Path.Value))
        ...
        protected ActionResult ListDirTree(string type)
        {
            ...
            string localPath = Server.MapWwwPath("~/");
        ...
        public ActionResult DownloadDir(string path)
        {
            ...
            string tmpZip = Server.MapWwwPath("~/FileManager/tmp/" + dirName + ".zip");
