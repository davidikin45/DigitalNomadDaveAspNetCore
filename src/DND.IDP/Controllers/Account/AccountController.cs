            ...
            if (ModelState.IsValid)
            {
                ...
                if (_userRepository.AreUserCredentialsValid(model.Username, model.Password))
                {
                    ...
                    else
                    {
                        ...
                        await HttpContext.SignInAsync(IdentityServerConstants.DefaultCookieAuthenticationScheme + ".2FA", new ClaimsPrincipal(id));

                        // send code...

                        var redirectToAdditionalFactorUrl =
                               Url.Action(nameof(AdditionalAuthenticationFactor),
                               new
                               {
