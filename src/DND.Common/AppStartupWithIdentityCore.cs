...
{
    public abstract class AppStartupWithIdentityCore<TIdentiyDbContext, TUser> : AppStartup
        where TIdentiyDbContext : DbContext
        where TUser : class
    {
        public AppStartupWithIdentityCore(ILoggerFactory loggerFactory, IConfiguration configuration, IHostingEnvironment hostingEnvironment)
