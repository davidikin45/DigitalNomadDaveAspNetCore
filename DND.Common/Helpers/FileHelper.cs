...
namespace DND.Common.Helpers
{
    public static class FileHelperExtensions
    {
        ...
        public static string GetAbsoluteUrlSlug(string folderId, string path, int width = 0, int height = 0, int size = 0, int maxWidth = 0, int maxHeight = 0, bool watermark = false)
        {
            return AbsoluteUrlSlug(FileHelper.GetFileInfo(folderId, path), width, height, size, maxWidth, maxHeight, watermark);
        }
        ...
        public static string VirtualPath(this FileInfo fileinfo)
        {
            return GetAbsoluteVirtualPath(fileinfo.FullName);
        }

        public static string GetAbsoluteVirtualPath(this string physicalPath)
        {
            string appRelativeVirtualPath = "~/" + physicalPath.Replace(Server.WwwRoot(), String.Empty).Replace("\\", "/");
    ...
    public static class FileHelper
    {
        ...
        public static bool FileExists(string folderId, string path)
        {
            return FileExists(GetWwwFolderPhysicalPathById(folderId) + path);
        }

        public static FileInfo GetFileInfo(string folderId, string path)
        {
            return GetFileInfo(GetWwwFolderPhysicalPathById(folderId) + path);
        }
        ...
        public static bool FolderExists(string folderId, string path)
        {
            return FolderExists(GetWwwFolderPhysicalPathById(folderId) + path);
        }
        ...
        public static DirectoryInfo GetDirectoryInfo(string folderId, string path)
        {
            return GetDirectoryInfo(GetWwwFolderPhysicalPathById(folderId) + path);
        }
        ...
        private static System.Drawing.Icon getIcon(string ext, bool large)
        {
            ...
            bool flag = WebHelper.ServerDetails.DLLExists("Icons.dll");
            if (ext == ".exe" && flag)
            {
                text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",2";
            }
            else if (ext == ".pdf" && flag)
            {
                text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",5";
            }
            else if ((ext == ".docx" || ext == ".doc") && flag)
            {
                text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",6";
            }
            else if (ext == ".xlsx" && WebHelper.ServerDetails.DLLExists("Icons.dll"))
            {
                text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",7";
            }
            else if ((ext == ".mp3" || ext == ".wav") && flag)
            {
                text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",8";
            }
            else
            {
                if (ext == ".mov")
                ...
                if (flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",9";
                    goto IL_485;
                }
                IL_1E0:
                if (ext == ".css" && flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",10";
                }
                else if (ext == ".js" && flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",11";
                }
                else if ((ext == ".zip" || ext == ".7zip") && flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",12";
                }
                else if (ext == ".txt" && flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",13";
                }
                else if ((ext == ".html" || ext == ".htm") && flag)
                {
                    text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",14";
                }
                else
                {
                    if (ext == ".vb")
                    ...
                    if (flag)
                    {
                        text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",15";
                        goto IL_485;
                    }
                    IL_33D:
                    if (ext == ".fb" && flag)
                    {
                        text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",16";
                    }
                    else if (ext == ".insta" && flag)
                    {
                        text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",17";
                    }
                    else if (ext == ".lin" && flag)
                    {
                        text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",18";
                    }
                    else if (ext == ".xml" && flag)
                    {
                        text2 = WebHelper.ServerDetails.DLLPath("Icons.dll") + ",19";
                    }
                    else if (ext == ".exe")
                    {
                        text2 = "shell32.dll,2";
        ...
        public static byte[] GetBytesByRelativePath(string relativePath)
        {
            string physicalPath = WebHelper.ServerDetails.GetPhysicalPath(relativePath);
            if (File.Exists(physicalPath))
            {
                string fileName = WebHelper.ServerDetails.GetFileName(physicalPath);
                string folder = WebHelper.ServerDetails.GetFolderName(physicalPath);
                return FileHelper.GetBytes(physicalPath);
            }
            return null;
        }
