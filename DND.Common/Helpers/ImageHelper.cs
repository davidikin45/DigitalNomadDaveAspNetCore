...
namespace DND.Common.Helpers
{
    public static class ImageHelperExtensions
    {
        ...
        public static JpegMetadata JpegMetaData(this FileInfo fileinfo)
        {
            return new JpegMetadata(fileinfo.FullName);
        }
        ...
        {
            return System.Web.MimeMapping.GetMimeMapping(fileinfo.Name).StartsWith("image/");
        }
        ...
        public static Bitmap ApplyWatermark(Image image, ImageHelper.Watermark location, int topBottomPadding, int leftRightPadding, string watermarkPath, ref bool applied)
        {
            switch (location)
            {
                case ImageHelper.Watermark.TopRight:
                    {
                        applied = true;
                        using (Image image2 = ImageHelper.GetImage(WebHelper.ServerDetails.GetPhysicalPath(watermarkPath)))
                        {
                ...
                case ImageHelper.Watermark.TopLeft:
                    {
                        applied = true;
                        using (Image image3 = ImageHelper.GetImage(WebHelper.ServerDetails.GetPhysicalPath(watermarkPath)))
                        {
                ...
                case ImageHelper.Watermark.BottomRight:
                    {
                        applied = true;
                        using (Image image4 = ImageHelper.GetImage(WebHelper.ServerDetails.GetPhysicalPath(watermarkPath)))
                        {
                ...
                case ImageHelper.Watermark.BottomLeft:
                    {
                        applied = true;
                        using (Image image5 = ImageHelper.GetImage(WebHelper.ServerDetails.GetPhysicalPath(watermarkPath)))
                        {
